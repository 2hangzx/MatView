...
`visualizeMatField` 用于从任意 MAT 文件中发现并加载符合规则的实数数值二维/三维数组，
以二维切片或三维等值面方式交互显示。对外只有 `visualizeMatField.m` 一个公共入口；
实现代码位于 `+matfield` MATLAB 包中。
...
│  ├─ Data.m                    MAT 路径解析、变量发现、加载、分类和数值校验
...
1. `visualizeMatField` 调用 `Input.parse`，统一识别两个可选位置参数和所有 Name-Value
   参数，并检查参数间的逐级依赖。
2. 若提供了 MAT 文件，`Data.resolveMatFile` 将相对路径解析为规范绝对路径。
3. 若未指定目标变量，`Data.listSelectableVariables` 只寻找合法二维/三维实数数值数组，
   然后进入 `SelectionView`；未指定文件时先保持变量选择和控制台禁用。
4. 变量确定后，`Data.loadTargetArray` 只加载目标顶层变量并沿点分字段路径取值；
...
- 数据：`Data`、`DataSize`、`FieldDimension`、`Variable`、`GlobalRange`。
- 配置：`Options`、`PlotMode`。
...
- 新增 MAT 变量规则：只修改 `Data`，不要在下拉框或渲染函数中重复判断。
...
测试覆盖空文件选择状态、变量选择、二维 Slice、三维 Slice、Volume、模式往返切换和
响应式布局档位。涉及视觉布局或三维鼠标交互的改动，自动测试通过后仍应使用真实可见
窗口进行一次人工检查。
