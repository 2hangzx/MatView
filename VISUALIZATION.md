...
查看器窗口启用 MATLAB 原生的 figure 工具栏。Volume 模式会在其下方再创建一排始终
可见的三维导航工具栏，并保留坐标区右上角的悬浮工具栏：
...
旋转与平移是互斥的左键模式；再次点击当前按钮会回到默认鼠标交互。放大、缩小和恢复
视图是即时操作，鼠标滚轮缩放仍然可用。调整等值面数量或范围并重新绘制时，会保留用户
当前的坐标范围、相机位置、观察目标、朝上方向、视角和投影方式。
